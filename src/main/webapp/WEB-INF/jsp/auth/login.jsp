  ...
  <form name="f" action="<c:url value='/j_spring_security_check'/>" method="POST" onsubmit='return validate();'>
    <table class="data">
      <tr>
        ...
        <td class="input"><input type='text' name='j_username' value='<c:if test="${not empty param.error}"><c:out value="${SPRING_SECURITY_LAST_EXCEPTION.authentication.principal}"/></c:if>'/></td>
      </tr>
    ...
    <div class="submit">
      <input name="submit" type="submit" value="<fmt:message key="auth.login_submit.label" />"/>
    </div>
  </form>
