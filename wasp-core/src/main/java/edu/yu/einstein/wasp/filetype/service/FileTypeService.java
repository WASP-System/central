...
public interface FileTypeService extends WaspService {
    ...
    /**
     * get the number of the file in the series. Always 0-based and 0 or null if
     * there is only one file. In the case where multiple files comprise a
     * component of a single file (e.g. separate FASTQ files for forward and
     * reverse reads in an Illumina run), each of these files will have the same
     * "file number" and must be discriminated by an additional piece of
     * metadata supplied by the file specific service. (e.g.
    ...
    /**
     * add a file group attribute, typically managed as a public static variable
     * in the specific file type service implementation (eg FastqService).
     * 
     * @param fg
     * @param attribute
     */
