...
public class SampleServiceImpl extends WaspServiceImpl implements SampleService {
	...
	public List<SampleSubtype> getSampleSubtypesForWorkflowByLoggedInUserRoles(
			Integer workflowId) {
		return getSampleSubtypesForWorkflowByRole(workflowId,
				authenticationService.getRoles(), null);
	}
	...
	public List<SampleSubtype> getSampleSubtypesForWorkflowByLoggedInUserRoles(
		...
		return getSampleSubtypesForWorkflowByRole(workflowId,
				authenticationService.getRoles(), sampleTypeIName);
	}
	...
	public void saveSampleWithAssociatedMeta(Sample sample) {
		...
		sampleMetaDao.updateBySampleId(sample.getSampleId(),
				sample.getSampleMeta());
	}
	...
	public List<SampleSubtype> getSampleSubtypesForWorkflowByRole(
		...
		for (WorkflowSampleSubtype wfsts : workflowDao.getWorkflowByWorkflowId(
				workflowId).getWorkflowSampleSubtype()) {
			SampleSubtype sts = wfsts.getSampleSubtype();
			if (sampleTypeIName == null
					|| sampleTypeIName.equals(sts.getSampleType().getIName())) {
				String[] includedRoles = new String[] {};
				String[] excludedRoles = new String[] {};
				try {
					includedRoles = MetaHelper.getMetaValue(sts.getIName(),
							"includeRoles", sts.getSampleSubtypeMeta()).split(
							",");
				} catch (MetadataException e) {
					// "includeRoles" meta not present
				}
				try {
					excludedRoles = MetaHelper.getMetaValue(sts.getIName(),
							"excludeRoles", sts.getSampleSubtypeMeta()).split(
							",");
				} catch (MetadataException e) {
					// "excludeRoles" meta not present
				}
				if ((includedRoles.length == 0 && excludedRoles.length == 0)
						|| (authenticationService.hasRoleInRoleArray(
								includedRoles, roles) && !authenticationService
								.hasRoleInRoleArray(excludedRoles, roles))) {
	...
	public boolean isSampleNameUniqueWithinJob(Sample sampleIn,
		...
		for (Sample sample : samplesInThisJob) {
			if (sampleIn.getSampleId() != null
					&& sample.getSampleId().intValue() == sampleIn
							.getSampleId().intValue())
				continue;
			if (sample.getSampleType().getIName().equals(sampleType.getIName())
					&& sampleIn.getName().equals(sample.getName())) {
				return false;
			}
		}
		return true;
	...
	public void sortSamplesBySampleName(List<Sample> samples) {
		...
		Collections.sort(samples, new SampleNameComparator());// sort by
																// sample's name
	}
	...
	public String convertReceiveSampleStatusForWeb(String internalStatus) {
		...
		} else if (internalStatus.equals("RECEIVED")
				|| internalStatus.equals("COMPLETED")
				|| internalStatus.equals("FINALIZED")) {
			return new String("RECEIVED");
		} else if (internalStatus.equals("WITHDRAWN")
				|| internalStatus.equals("ABANDONED")) {
	 ...
	 */
	@Override
	public boolean updateSampleReceiveStatus(final Sample sample,
			final String status) {
		// TODO: Write test!!
	 ...
	 */
	@Override
	public boolean submittedSampleHasBeenProcessedByFacility(final Sample sample) {// should
																					// but
																					// doesn't
																					// really
																					// check
		...
		if (sample.getSourceSampleId().size() > 0) {/*
													 * submitted sample is a
													 * user-submitted library
													 * that has been placed onto
													 * a flow cell or a
													 * user-submitted
													 * macromolecule that has
													 * been used to generate a
													 * library
													 */
	...
	public Adaptor getLibraryAdaptor(Sample library) {
		...
		String[] stringList = areaList.split("[\\s,]+");// separates on comma or
														// whitespace
		...
		try {
			adaptorId = MetaHelper.getMetaValue(area, "adaptor",
					library.getSampleMeta());
		} catch (MetadataException me) {
			logger.warn("Unable to identify adaptor for libraryId "
					+ library.getSampleId());
		}
		if (!adaptorId.equals("")) {
	...
	public List<Sample> getAvailableFlowCells() {
		...
		SampleType sampleType = sampleTypeDao
				.getSampleTypeByIName("platformunit");
		if (sampleType == null || sampleType.getSampleTypeId() == null
				|| sampleType.getSampleTypeId().intValue() == 0) {
	...
	public List<Sample> getAvailableAndCompatibleFlowCells(Job job) {
		...
		for (Sample flowCell : availableFlowCells) {
			for (SampleSubtypeResourceCategory ssrc : flowCell
					.getSampleSubtype().getSampleSubtypeResourceCategory()) {
				for (JobResourcecategory jrc : job.getJobResourcecategory()) {
					if (ssrc.getResourcecategoryId().intValue() == jrc
							.getResourcecategoryId().intValue()) {
	...
	public Sample getPlatformUnitForCell(Sample cell)
		...
		if (!cell.getSampleType().getIName().equals("cell")) {
			throw new SampleTypeException(
					"Expected 'cell' but got Sample of type '"
							+ cell.getSampleType().getIName() + "' instead.");
		}
		if (cell.getSampleSource() == null || cell.getSampleSource().isEmpty())
			throw new SampleParentChildException("Cell '"
					+ cell.getSampleId().toString()
					+ "' is associated with no flowcells");
		if (cell.getSampleSource().size() > 1)
			throw new SampleParentChildException("Cell '"
					+ cell.getSampleId().toString()
					+ "' is associated with more than one flowcell");
	...
	public Map<Integer, Sample> getIndexedCellsOnPlatformUnit(
		...
		if (!platformUnit.getSampleType().getIName().equals("platformunit")) {
			throw new SampleTypeException(
					"Expected 'platformunit' but got Sample of type '"
							+ platformUnit.getSampleType().getIName()
							+ "' instead.");
		}
	...
	public void addCellToPlatformUnit(Sample platformUnit, Sample cell,
		...
		if (sampleSourceDao.findByMap(sampleSourceQuery) != null)
			throw new SampleIndexException(
					"index '"
							+ index
							+ "' already assigned to a cell associated with this platform unit");
	...
	/**
	 * Index is essentially a a wrapper around an int. It is required (instead
	 * of Integer) because Integer is immutable and so it's value passed as a
	 * parameter, when 'changed' within a method cannot be used as the modified
	 * version outside that method.
	 * 
	 * @author asmclellan
	 * 
	 */
	...
	/**
	 * Returns list of Samples on a cell. If control libraries are spiked in,
	 * these are also returned. Also takes a parameter maxIndex (can be null)
	 * which returns the maximum index sampleSource index value found
	 * 
	 * @param cell
	 * @param maxIndex
	 * @return
	 * @throws SampleTypeException
	 */
	private List<Sample> getLibrariesOnCell(Sample cell, Index maxIndex)
			throws SampleTypeException {
		if (!cell.getSampleType().getIName().equals("cell")) {
			throw new SampleTypeException(
					"Expected 'cell' but got Sample of type '"
		...
		for (SampleSource ss : cell.getSampleSource()) {
			...
			if (!library.getSampleType().getIName().equals("library")
					&& !library.getSampleType().getIName()
							.equals("controlLibrarySample")) {
				throw new SampleTypeException(
						"Expected 'library' but got Sample of type '"
								+ cell.getSampleType().getIName()
								+ "' instead.");
			}
			if (maxIndex != null && ss.getIndex() != null
					&& ss.getIndex() > maxIndex.getValue())
	...
	public List<Sample> getLibrariesOnCellWithoutControls(Sample cell)
		...
		for (Sample library : getLibrariesOnCell(cell)) {
			if (!library.getSampleSubtype().getIName()
					.equals("controlLibrarySample"))
	...
	public void addLibraryToCell(Sample cell, Sample library,
		...
		if (!library.getSampleType().getIName().equals("library")) {
			throw new SampleTypeException(
					"Expected 'library' but got Sample of type '"
							+ library.getSampleType().getIName() + "' instead.");
		}
		/*
		 * (1) identify the barcode sequence on the library being added. If
		 * problem then terminate. (2) if the library being added has a barcode
		 * that is NONE, and the lane contains ANY OTHER LIBRARY, then
		 * terminate. (3) identify barcode of libraries already on lane; if
		 * problem, terminate. Should also get their jobIds. (4) if the lane
		 * already has a library with a barcode of NONE, then terminate (5) if
		 * the library being added has a bardcode that is something other than
		 * NONE (meaning a real barcode sequence) AND if a library already on
		 * the lane has that same barcode, then terminate. (6) do we want to
		 * maintain only a single jobId for a lane???
		 */

		// case 1: identify the adaptor barcode for the library being added;
		// it's barcode is either NONE (no multiplexing) or has some more
		// interesting barcode sequence (for multiplexing, such as AACTG)
		Adaptor adaptorOnLibraryBeingAdded = null;
		try {
			adaptorOnLibraryBeingAdded = adaptorDao
					.getAdaptorByAdaptorId(Integer.valueOf(MetaHelper
							.getMetaValue("genericLibrary", "adaptor",
									library.getSampleMeta())));
		} catch (NumberFormatException e) {
			throw new MetadataException(
					"Cannot convert genericLibrary.adaptor meta result to Integer: "
							+ e.getMessage());
		}

		if (adaptorOnLibraryBeingAdded == null
				|| adaptorOnLibraryBeingAdded.getAdaptorId() == null) {
			throw new SampleException("No adaptor associated with library");
		} else if (adaptorOnLibraryBeingAdded.getBarcodesequence() == null
				|| adaptorOnLibraryBeingAdded.getBarcodesequence().equals("")) {
			throw new SampleException("Library adaptor has no barcode");
		}
		Index index = new Index();
		List<Sample> libraries = this.getLibrariesOnCell(cell, index);
		index.increment();
		String barcodeOnLibBeingAdded = new String(
				adaptorOnLibraryBeingAdded.getBarcodesequence());

		// case 2: dispense with this easy check
		if (barcodeOnLibBeingAdded.equals("NONE") && libraries != null
				&& libraries.size() > 0) { // case 2: the library being added
											// has a barcode of "NONE" AND the
											// lane to which user wants to add
											// this library already contains one
											// or more libraries (such action is
											// prohibited)
			throw new SampleMultiplexException(
					"Cannot add more than one sample to cell if not multiplexed. Input library has barcode 'NONE'.");
		}
		...
		if (libraries != null) {
			for (Sample libraryAlreadyOnLane : libraries) {
				...
				try {
					adaptorOnLane = adaptorDao.getAdaptorByAdaptorId(Integer
							.valueOf(MetaHelper.getMetaValue("genericLibrary",
									"adaptor",
									libraryAlreadyOnLane.getSampleMeta())));
				} catch (NumberFormatException e) {
					throw new MetadataException(
							"Library already on lane: Cannot convert genericLibrary.adaptor meta result to Integer: "
									+ e.getMessage());
				}

				if (adaptorOnLane == null
						|| adaptorOnLane.getAdaptorId() == null) {
					throw new SampleException(
							"Library already on lane : No adaptor associated with library");
				} else if (adaptorOnLane.getBarcodesequence() == null
						|| adaptorOnLane.getBarcodesequence().equals("")) {
					throw new SampleException(
							"Library already on lane: adaptor has no barcode");
				} else if (adaptorOnLane.getBarcodesequence().equals("NONE")) {
					throw new SampleMultiplexException(
							"Library already on lane: Cannot add more than one sample to cell if not multiplexed. Library has barcode 'NONE'");
				} else if (adaptorOnLane.getBarcodesequence().equals(
						barcodeOnLibBeingAdded)) {
					throw new SampleMultiplexException(
							"Library already on lane: has same barcode as input library");
				} else {
					// TODO: confirm library is really part of this jobId. For
					// now do nothing. If Einstein, then terminate (lane
					// restricted to libraries from single job)
				}
			}
		}
		...
		newSampleSource = sampleSourceDao.save(newSampleSource);// capture the
																// new
																// samplesourceid

		try {
			MetaHelper metaHelper = new MetaHelper("LibraryOnCell",
					SampleSourceMeta.class);
			metaHelper.setMetaValueByName("libConcInLanePicoM",
					libConcInLanePicoM.toString());
			metaHelper.setMetaValueByName("jobId", library.getJob().getJobId()
					.toString());
			sampleSourceMetaDao.updateBySampleSourceId(
					newSampleSource.getSampleSourceId(),
					(List<SampleSourceMeta>) metaHelper.getMetaList());
		} catch (MetadataException e) {
	...
	public List<Sample> platformUnitsAwaitingLibraries() {
		...
		if (task == null || task.getTaskId() == null) {
			// waspErrorMessage("platformunit.taskNotFound.error"); maybe throw
			// exception?????
		}
		...
		Set<Sample> samples = new HashSet<Sample>();// use set to avoid
													// duplicates
		for (State state : stateList) {
			List<Statesample> statesampleList = state.getStatesample();
			for (Statesample statesample : statesampleList) {
				if (statesample.getSample().getSampleType().getIName()
						.equals("platformunit")) {
		...
		return new ArrayList<Sample>(samples);// return as list rather than as
												// set
	}
	...
	public List<SampleSubtype> getSampleSubtypesBySampleTypeIName(
		...
		SampleType sampleType = sampleTypeDao
				.getSampleTypeByIName(sampleTypeIName);
		if (sampleType == null || sampleType.getSampleTypeId() == null
				|| sampleType.getSampleTypeId().intValue() == 0) {
			throw new SampleTypeException("SampleType not found: iname = "
					+ sampleTypeIName);
		}
		...
		return sampleSubtypeDao.findByMapDistinctOrderBy(filterMap, null,
				orderByColumnNames, "asc");
	...
	public SampleSubtype getSampleSubtypeById(Integer sampleSubtypeId) {
		...
		return sampleSubtypeDao
				.getSampleSubtypeBySampleSubtypeId(sampleSubtypeId.intValue());
	}
	...
	public boolean sampleSubtypeIsSpecificSampleType(
		...
		if (sampleTypeIName == null || sampleSubtype == null
				|| sampleSubtype.getSampleType() == null
				|| sampleSubtype.getSampleType().getIName() == null) {
			return false;
		}
	...
	public boolean sampleIsSpecificSampleType(Sample sample,
		...
		if (sampleTypeIName == null || sample == null
				|| sample.getSampleType() == null
				|| sample.getSampleType().getIName() == null) {
			return false;
		}
	...
	public boolean sampleIsSpecificSampleSubtype(Sample sample,
		...
		if (sampleSubtypeIName == null || sample == null
				|| sample.getSampleSubtype() == null
				|| sample.getSampleSubtype().getIName() == null) {
			return false;
		}
	...
	public boolean sampleIdIsInDatabase(Integer sampleId) {
		...
		return sample.getSampleId() != null
				&& sample.getSampleId().intValue() > 0 ? true : false;
	}
	...
	public boolean sampleIsPlatformUnit(Sample sample) {
		...
		if ("platformunit".equals(sample.getSampleType().getIName())
				&& "platformunit".equals(sample.getSampleSubtype()
						.getSampleType().getIName())) {
			return true;
		}
		return false;
	}
	...
	public Sample getPlatformUnit(Integer sampleId)
		...
		if (sampleId == null) {
			throw new NumberFormatException("SampleId is null");
		}

		Sample sample = sampleDao.getSampleBySampleId(sampleId.intValue());
		if (!sampleIdIsValid(sample)) {
			throw new SampleException("Sample with sampleId of "
					+ sampleId.intValue() + " not in database.");
		} else if (!this.sampleIsSpecificSampleType(sample, "platformunit")) {
			throw new SampleTypeException("Sample with sampleId of "
					+ sampleId.intValue() + " not of sampleType platformunit.");
		} else if (!this.sampleSubtypeIsSpecificSampleType(
				sample.getSampleSubtype(), "platformunit")) {
			throw new SampleSubtypeException("Sample with sampleId of "
					+ sampleId.intValue()
					+ " not of sampleSubtype platformunit.");
		}
		// could have used this.sampleIsPlatformUnit(sample) as well for the two
	...
	public SampleSubtype getSampleSubtypeConfirmedForPlatformunit(
		...
		if (sampleSubtypeId == null) {
			throw new NumberFormatException("SampleSubtypeId is null");
		}

		SampleSubtype sampleSubtype = sampleSubtypeDao
				.getSampleSubtypeBySampleSubtypeId(sampleSubtypeId.intValue());
		if (sampleSubtype == null || sampleSubtype.getSampleSubtypeId() == null
				|| sampleSubtype.getSampleSubtypeId().intValue() <= 0) {
			throw new SampleSubtypeException(
					"SampleSubtype with sampleSubtypeId of "
							+ sampleSubtypeId.intValue() + " not in database.");
		} else if (!this.sampleSubtypeIsSpecificSampleType(sampleSubtype,
				"platformunit")) {
			throw new SampleSubtypeException(
					"SampleSubtype with sampleSubtypeId of "
							+ sampleSubtypeId.intValue()
							+ " not of sampletype platformunit.");
		}
	...
	public List<Integer> getNumberOfCellsListForThisTypeOfPlatformUnit(
		...
		if (!sampleSubtypeIsSpecificSampleType(sampleSubtype, "platformunit")) {
			throw new SampleSubtypeException("SampleSubtype with Id of "
					+ sampleSubtype.getSampleSubtypeId().toString()
					+ " is not platformunit");
		}
		...
		List<SampleSubtypeMeta> ssMetaList = sampleSubtype
				.getSampleSubtypeMeta();
		for (SampleSubtypeMeta ssm : ssMetaList) {
			if (ssm.getK().indexOf("maxCellNumber") > -1) {
				try {
					maxCellNumber = new Integer(ssm.getV()); // could throw
																// NumberFormatException
				} catch (Exception e) {
					throw new SampleSubtypeException(
							"maxCellNumber value invalid for SampleSubtype with Id of "
									+ sampleSubtype.getSampleTypeId()
											.intValue());
				}
			}
			if (ssm.getK().indexOf("multiplicationFactor") > -1) {
				try {
					multiplicationFactor = new Integer(ssm.getV()); // could
																	// throw
																	// NumberFormatException
				} catch (Exception e) {
					throw new SampleSubtypeException(
							"multiplicationFactor value invalid for SampleSubtype with Id of "
									+ sampleSubtype.getSampleTypeId()
											.intValue());
				}
			}
		}
		if (maxCellNumber == null) {
			throw new SampleSubtypeException(
					"maxCellNumber not found for SampleSubtype with Id of "
							+ sampleSubtype.getSampleTypeId().intValue());
		} else if (maxCellNumber.intValue() <= 0) {
			throw new SampleSubtypeException(
					"maxCellNumber value invalid for SampleSubtype with Id of "
							+ sampleSubtype.getSampleTypeId().intValue());
		}
	...
	public boolean requestedReductionInCellNumberIsProhibited(
		...
		Map<Integer, Sample> indexedCellMap = this
				.getIndexedCellsOnPlatformUnit(platformUnitInDatabase);// throws
																		// exception
		Integer numberOfLanesInDatabase = indexedCellMap.size();
		if (numberOfLanesInDatabase.intValue() <= numberOfLanesRequested
				.intValue()) {// no loss of lanes, so return false, as action
								// not prohibited
			return false;
		}

		// user asking to reduce number of lanes
		// must check for presence of libraries on those lanes that user seems
		// to want to remove. If any found, return true.
		for (int i = numberOfLanesRequested.intValue() + 1; i <= numberOfLanesInDatabase
				.intValue(); i++) {
			Integer index = new Integer(i);
			Sample cell = indexedCellMap.get(index);
			if (cell == null) {
				// unexpected problem; indexes not ordered
				throw new SampleException("No cell found for platformUnitId "
						+ platformUnitInDatabase.getSampleId().intValue()
						+ " and cell index " + i);
			}
			List<Sample> libraryList = null;
			libraryList = this.getLibrariesOnCell(cell);// throws exception
			if (libraryList != null && libraryList.size() > 0) {// found at
																// least one
																// library
				return true;
			}
	...
	public void createUpdatePlatformUnit(Sample platformUnit,
		...
		if (sampleIsInDatabase(platformUnit)) {// this is an update of an
												// existing record
			pu = platformUnit;
			if (!sampleIsPlatformUnit(platformUnit)) {
				throw new SampleException(
						"Sample with Id of "
								+ pu.getSampleId().toString()
								+ " unexpectedly NOT a platformUnit either in sampletype or samplesubtype");
			}

			// check this first, since it could throw an exception, so no need
			// to proceed with the update unless this is OK
			numberOfLanesInDatabase = this
					.getNumberOfIndexedCellsOnPlatformUnit(pu);
			if (numberOfLanesInDatabase == null
					|| numberOfLanesInDatabase.intValue() <= 0) {// should never
																	// be 0
																	// lanes on
																	// a
																	// platformunit
				throw new SampleException(
						"lanecount in database is not valid for platformunit with Id "
								+ pu.getSampleId().intValue());
			}

			// if update, determine whether platformunit name has been changed
			if (barcodeName != null
					&& !barcodeName.equals(platformUnit.getName())) {
				platformUnitNameHasBeenChanged = true;
			}
			System.out.println("The name of the pu sample is : "
					+ platformUnit.getName());
		...
		if (!this.sampleSubtypeIsSpecificSampleType(sampleSubtype,
				"platformunit")) {
			throw new SampleSubtypeException("SampleSubtype with ID of "
					+ sampleSubtype.getSampleSubtypeId().toString()
					+ " is unexpectedly not SampleType of platformunit");
		}
		sampleTypeForPlatformUnit = sampleTypeDao
				.getSampleTypeByIName("platformunit");
		if (sampleTypeForPlatformUnit == null
				|| sampleTypeForPlatformUnit.getSampleTypeId() == null
				|| sampleTypeForPlatformUnit.getSampleTypeId().intValue() <= 0) {
			throw new SampleTypeException(
					"SampleType of type platformunit unexpectedly not found");
		}
		sampleTypeForCell = sampleTypeDao.getSampleTypeByIName("cell");
		if (sampleTypeForCell == null
				|| sampleTypeForCell.getSampleTypeId() == null
				|| sampleTypeForCell.getSampleTypeId().intValue() <= 0) {
			throw new SampleTypeException(
					"SampleType of type cell unexpectedly not found");
		}

		if (numberOfLanesRequested == null
				|| numberOfLanesRequested.intValue() <= 0) {
			throw new SampleException(
					"Number of lanes requested not valid value");
		} else {// confirm numberOfLanesRequested is a valid value for this
				// subtype of platformUnit
			List<Integer> numberOfCellsList = this
					.getNumberOfCellsListForThisTypeOfPlatformUnit(sampleSubtype);
			boolean foundIt = false;
			for (Integer numberOfCellsAllowed : numberOfCellsList) {
				if (numberOfCellsAllowed.intValue() == numberOfLanesRequested
						.intValue()) {
					foundIt = true;
					break;
				}
			...
			if (!foundIt) {
				throw new SampleException(
						"Number of lanes requested is not compatible with the requested sampleSubtype");
			}
		}

		if (numberOfLanesRequested.intValue() >= numberOfLanesInDatabase
				.intValue()) {// request to add lanes or no change in lane
								// number, so not a problem
			;
		} else if (numberOfLanesRequested.intValue() < numberOfLanesInDatabase
				.intValue()) {// request to remove lanes; a potential problem if
								// libraries are on the lanes to be removed
			// perform next test
			if (this.requestedReductionInCellNumberIsProhibited(pu,
					numberOfLanesRequested)) {// value of true means libraries
												// are assigned to those lanes
												// being asked to be removed.
												// Prohibit this action and
												// inform user to first remove
												// those libraries from the
												// lanes being requested to be
												// removed
				throw new SampleException(
						"Sample Exception during platform unit update: Action not permitted at this time. To reduce the number of lanes, remove libraries on the lanes that will be lost.");
			}
		}
		...
		if (barcodeName == null || "".equalsIgnoreCase(barcodeName)) {
			...
			if ("create".equals(action) /*
										 * this is new platformUnit, and barcode
										 * name already used, so prevent
										 */
					|| ("update".equals(action) && !barcodeName
							.equalsIgnoreCase(pu.getSampleBarcode().get(0)
		...
		try {
			pu.setName(barcodeName);// sample.name will be set to the barcode
									// name; as per Andy 9-28-12

			User me = authenticationService.getAuthenticatedUser();
			pu.setSubmitterUserId(me.getUserId());

			pu.setSampleSubtypeId(sampleSubtype.getSampleSubtypeId());// sampleSubtype
																		// is a
																		// parameter
			...
			if (platformUnitDb == null || platformUnitDb.getSampleId() == null
					|| platformUnitDb.getSampleId().intValue() <= 0) {
				throw new SampleException(
						"new platform unit unexpectedly not saved");
			}
			sampleMetaDao.updateBySampleId(platformUnitDb.getSampleId(),
					sampleMetaList); // persist the metadata; no way to check as
										// this returns void

			// barcode
			List<SampleBarcode> sampleBarcodeList = platformUnitDb
					.getSampleBarcode();// any barcodes exist for this platform
										// unit?
			if (sampleBarcodeList != null && sampleBarcodeList.size() > 0) {// this
																			// is
																			// an
																			// update
				SampleBarcode sampleBarcode = sampleBarcodeList.get(0);
				Barcode existingBarcode = sampleBarcode.getBarcode();
				existingBarcode.setBarcode(barcodeName);// update the
														// barcodeName
				Barcode barcodeDb = this.barcodeDao.save(existingBarcode);
				if (barcodeDb == null || barcodeDb.getBarcodeId() == null
						|| barcodeDb.getBarcodeId().intValue() <= 0) {
					throw new SampleException(
							"updated barcode unexpectedly not saved");
				}
				...
				Barcode barcodeDb = this.barcodeDao.save(barcodeObject);// save
																		// new
																		// barcode
																		// in db
				if (barcodeDb == null || barcodeDb.getBarcodeId() == null
				...
				sampleBarcode.setBarcodeId(barcodeDb.getBarcodeId()); // set new
																		// barcodeId
																		// in
																		// samplebarcode
				sampleBarcode.setSampleId(platformUnitDb.getSampleId());
				SampleBarcode sampleBarcodeDb = this.sampleBarcodeDao
						.save(sampleBarcode);
				if (sampleBarcodeDb == null
						|| sampleBarcodeDb.getSampleBarcode() == null
						|| sampleBarcodeDb.getSampleBarcode().intValue() <= 0) {
					throw new SampleException(
							"new samplebarcode in update area unexpectedly not saved");
				}
			}

			// The name of the platformunit (such as 102A4; which is the barcode
			// name) is also part of the name of the cell (102A4/1)
			// and if the name of the platformunit is being updated, then the
			// names of the cells also have to be updated
			// Do this only if needed: action == update [since no cells yet
			// exist] and the name has been changed (and cells exist - which
			// they should).
			if (action.equals("update") && platformUnitNameHasBeenChanged) {
				List<SampleSource> sampleSourceList = platformUnitDb
						.getSampleSource();
				if (sampleSourceList != null && sampleSourceList.size() > 0) {// should
																				// be
																				// true
																				// for
																				// all
					...
					for (SampleSource ss : sampleSourceList) {
						...
						if (!cell.getSampleType().getIName().equals("cell")) {
							throw new SampleTypeException(
									"Expected 'cell' while updating cell name but got Sample of type '"
											+ cell.getSampleType().getIName()
											+ "' instead.");
						}
						Integer index = ss.getIndex();
						...
						if (cellDb == null || cellDb.getSampleId() == null
								|| cell.getSampleId().intValue() <= 0) {
							throw new SampleException(
									"updated cell unexpectedly not saved during update of cell name");
						}
					}
				}
			}
			...
			// lanes
			// if create new platformunit, or add/remove additional lanes during
			// and update
			if (numberOfLanesRequested.intValue() > numberOfLanesInDatabase
					.intValue()) {// add lanes; can be create new record (where
									// numberOfLanesInDatabase = 0) or an update
									// to add some more lanes

				for (int i = numberOfLanesInDatabase + 1; i <= numberOfLanesRequested; i++) {// add
																								// additional
																								// lanes
					...
					if (cellDb == null || cellDb.getSampleId() == null
							|| cell.getSampleId().intValue() <= 0) {
						throw new SampleException(
								"new cell unexpectedly not saved during create or update of platformunit");
					}
					...
					SampleSource sampleSourceDb = this.sampleSourceDao
							.save(sampleSource);
					if (sampleSourceDb == null
							|| sampleSourceDb.getSampleId() == null
							|| sampleSourceDb.getSampleId().intValue() <= 0) {
						throw new SampleException(
								"new samplesource unexpectedly not saved during create or update of platformunit");
					}
				}
			} else if (numberOfLanesRequested.intValue() < numberOfLanesInDatabase
					.intValue()) {// update requests to remove lanes; above we
									// already confirmed that this will NOT
									// result in loss of info with this call
									// (if(this.requestedReductionInCellNumberIsProhibited(pu,
									// numberOfLanesRequested))

				// get the list
				// Map<Integer, Sample> indexedCellMap =
				// sampleService.getIndexedCellsOnPlatformUnit(platformUnitInDatabase);
				for (SampleSource ss : platformUnitDb.getSampleSource()) {
					...
					Integer index = ss.getIndex();
					if (index.intValue() >= numberOfLanesRequested.intValue() + 1
							&& index.intValue() <= numberOfLanesInDatabase
									.intValue()) {
						// check for present of any libraries on these cells
						// (just as a final fail safe mechanism, as this was
						// tested above)
						List<Sample> libraryList = null;
						libraryList = this.getLibrariesOnCell(cell);// throws
																	// exception
						if (libraryList != null && libraryList.size() > 0) {// found
																			// at
																			// least
																			// one
																			// library
							throw new SampleException(
									"Cell "
											+ cell.getSampleId().intValue()
											+ "unexpectedly has "
											+ libraryList.size()
											+ " libraries on it. Unable to remove this lane");
						}
						// first deletes each pu-cell link and its meta (if any)
						// AND THEN ALSO deletes the cell and the cell's meta
						// (if any)
	...
	public void deletePlatformUnit(Integer platformUnitId)
		...
		try {
			Sample platformUnit = this.getPlatformUnit(platformUnitId);// throws
																		// exceptions
																		// if
																		// not
																		// valid
			...
			for (SampleSource puCellLink : platformUnit.getSampleSource()) {// represents
																			// pu-cell
																			// link
				Sample cell = puCellLink.getSourceSample();// cell is the lane
				if (!cell.getSampleType().getIName().equals("cell")) {// confirm
																		// its a
																		// cell
					throw new SampleTypeException(
							"Expected 'cell' but got Sample of type '"
				...
				for (SampleSource cellLibraryLink : cell.getSampleSource()) {// each
																				// cellLibraryLink
																				// represents
																				// a
																				// cell-library
					...
					// deletes each cell-lib link, along with the links metadata
					// (if any)
					// NOTE: the library itself, of course, remains untouched
					this.removeLibraryFromCellOfPlatformUnit(cellLibraryLink);
				}
				// first deletes each pu-cell link and its meta (if any) AND
				// THEN ALSO deletes the cell and the cell's meta (if any)
				this.deleteCellFromPlatformUnit(puCellLink);
			}
			// deletes the platformunit itself and its meta (if any) and its
	...
	private void removeLibraryFromCellOfPlatformUnit(
			SampleSource cellLibraryLink) throws SampleTypeException {
		if (!cellLibraryLink.getSourceSample().getSampleType().getIName()
				.equals("library")) {
			throw new SampleTypeException(
					"Expected 'library' but got Sample of type '"
							+ cellLibraryLink.getSourceSample().getSampleType()
									.getIName() + "' instead.");
		}
		if (!cellLibraryLink.getSample().getSampleType().getIName()
				.equals("cell")) {
			throw new SampleTypeException(
					"Expected 'cell' but got Sample of type '"
							+ cellLibraryLink.getSample().getSampleType()
									.getIName() + "' instead.");
		}
		this.deleteSampleSourceAndItsMeta(cellLibraryLink);// currently the
															// cellLibraryLink
															// meta represents
															// the pM applied
															// and the jobId
	...
	private void deleteCellFromPlatformUnit(SampleSource puCellLink)
			throws SampleTypeException {
		Sample cell = puCellLink.getSourceSample();// cell is the lane
		if (!cell.getSampleType().getIName().equals("cell")) {
			throw new SampleTypeException(
		...
		if (!puCellLink.getSample().getSampleType().getIName()
				.equals("platformunit")) {
			throw new SampleTypeException(
					"Expected 'platformunit' but got Sample of type '"
							+ puCellLink.getSample().getSampleType().getIName()
							+ "' instead.");
		}
		this.deleteSampleSourceAndItsMeta(puCellLink);// first, remove the
														// samplesource link
														// (currently this is no
														// meta here, but if in
														// the future there is,
														// it will be taken care
														// of automatically)
		this.deleteSampleAndItsMeta(cell);// second, remove the cell itself
											// (currently this is no meta here,
											// but if in the future there is, it
											// will be taken care of
											// automatically)
	}

	private void deletePlatformUnit(Sample platformUnit)
			throws SampleTypeException {
		if (!platformUnit.getSampleType().getIName().equals("platformunit")) {
			throw new SampleTypeException(
					"Expected 'platformunit' but got Sample of type '"
		...
		this.deleteSampleAndItsMeta(platformUnit);// currently, meta includes
													// readlength, readType,
													// comments
	}
