...
<div id="loading_dialog-modal" title="<fmt:message key="waspIlluminaPlugin.displayQc_loadingTitle.label" />"  >
	...
	<tr>
	<td><img src="/wasp/images/waspIlluminaHiSeq/postRunQC/spinner.gif" align="left" border="0" ></td>
...
<c:forEach items="${fileHandlesByName.keySet()}" var="fileName" varStatus="status" >
	<img src='<wasp:url fileAccessor = "${fileHandlesByName.get(fileName)}" />' alt='Cycle <c:out value="${status.count}" /> FWHM Plot <c:out value="${fileName}" />' class='preloadHidden' />
</c:forEach>
