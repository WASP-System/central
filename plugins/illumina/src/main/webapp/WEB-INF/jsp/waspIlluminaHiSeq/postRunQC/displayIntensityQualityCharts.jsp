...
<c:forEach items="${fileHandlesByName.keySet()}" var="fileName" varStatus="status" >
	<img src='<wasp:url fileAccessor = "${fileHandlesByName.get(fileName)}" />' alt='Cycle <c:out value="${status.count}" /> Intensity Plot <c:out value="${fileName}" />' class='preloadHidden' />
</c:forEach>
