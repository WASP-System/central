				...
				<td>
					<div class="radio-jquery-ui">
						<input type="radio" id="passL<c:out value="${index}" />" name="radioL<c:out value="${index}" />" size="25" value="1" 
							<c:if test="${not empty(existingQcValuesIndexed) && not empty(existingQcValuesIndexed).get(index)) && not empty(existingQcValuesIndexed).get(index).isPassedQc()) && existingQcValuesIndexed.get(index).isPassedQc() == true}"> checked="checked"</c:if> />
						...
						<input type="radio" id="failL<c:out value="${index}" />" name="radioL<c:out value="${index}" />" size="25" value="0" 
							<c:if test="${not empty(existingQcValuesIndexed) && not empty(existingQcValuesIndexed).get(index)) && not empty(existingQcValuesIndexed).get(index).isPassedQc()) && existingQcValuesIndexed.get(index).isPassedQc() == false}"> checked="checked"</c:if>/>
