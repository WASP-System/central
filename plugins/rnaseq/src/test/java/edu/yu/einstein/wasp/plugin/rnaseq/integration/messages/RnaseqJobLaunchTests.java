...
import edu.yu.einstein.wasp.plugin.rnaseq.plugin.RnaseqPlugin;
...
@ContextConfiguration(locations={"/rnaseq-test-launch-context.xml","/flows/rnaseq.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the RnaseqPlugin.FLOW_NAME batch job flow (defined in /flows/rnaseq.mainFlow.v1.xml)
 * @author 
 * 
 */
public class RnaseqJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(RnaseqJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the RnaseqPlugin.FLOW_NAME job flow. 
	 ...
	 */
	@Test (groups = "rnaseq-tests")
	public void testSuccessfulRnaseqJobLaunch() throws Exception{
		try{
			Map<String, String> jobParameters = new HashMap<String, String>();
			// The rnaseq flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(RnaseqPlugin.FLOW_NAME, jobParameters) );
			...
			// The RnaseqPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the RnaseqPlugin.FLOW_NAME job flow.
			...
			ExitStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(RnaseqPlugin.FLOW_NAME)).getExitStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleRnaseqStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleRnaseqStatusMessageTemplate.
		if (SimpleRnaseqStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Rnaseq messages only
	}
