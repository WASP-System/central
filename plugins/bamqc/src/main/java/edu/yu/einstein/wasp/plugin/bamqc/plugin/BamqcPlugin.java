 ...
 * @author 
 */
package edu.yu.einstein.wasp.plugin.bamqc.plugin;
 ...
 * @author 
 */
public class BamqcPlugin extends WaspPlugin 
		implements 
			BatchJobProviding,
			FileDataTabViewing,
			ClientMessageI {
	...
	@Autowired
	@Qualifier("bamqc")
	private Software bamqc;
	
	public static final String FLOW_NAME = "edu.yu.einstein.wasp.plugin.bamqc.mainFlow";

	public BamqcPlugin(String iName, Properties waspSiteProperties, MessageChannel channel) {
	...
	private Message<String> helloWorldHelp() {
		String mstr = "\nBamqc plugin: hello world!\n" +
				"wasp -T bamqc -t helloWorld\n";
	...
	private Message<String> launchTestFlowHelp() {
		String mstr = "\nBamqc plugin: launch the test flow.\n" +
				"wasp -T bamqc -t launchTestFlow -m \'{id:\"1\"}\'\n";
		return MessageBuilder.withPayload(mstr).build();
	}
	...
	public Hyperlink getDescriptionPageHyperlink() {
		return new Hyperlink("bamqc.hyperlink.label", "/bamqc/displayDescription.do");
	}
