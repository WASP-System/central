 ...
 * 
 */
public class SimpleBamqcStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleBamqcStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, BamqcMessageType.Bamqc);
	}
	
	public SimpleBamqcStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(BamqcMessageType.Bamqc))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct BamqcMessageType
	 * @param message
	 * @return
	 */
