 ...
 * 
 */
public class SimpleBioanalyzerStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleBioanalyzerStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, BioanalyzerMessageType.BIOANALYZER);
	}
	
	public SimpleBioanalyzerStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(BioanalyzerMessageType.BIOANALYZER))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct BioanalyzerMessageType
	 * @param message
	 * @return
	 */
	public static boolean isMessageOfCorrectType(Message<?> message) {
		return message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) &&  
				message.getHeaders().get(WaspMessageType.HEADER_KEY).equals(BioanalyzerMessageType.BIOANALYZER);
	}
	
	@Override
	public SimpleBioanalyzerStatusMessageTemplate getNewInstance(WaspStatusMessageTemplate messageTemplate){
		SimpleBioanalyzerStatusMessageTemplate newTemplate = new SimpleBioanalyzerStatusMessageTemplate();
