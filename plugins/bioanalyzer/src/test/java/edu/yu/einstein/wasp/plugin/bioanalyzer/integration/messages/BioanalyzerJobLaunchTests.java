 ...
 * @author 
 */
package edu.yu.einstein.wasp.plugin.bioanalyzer.integration.messages;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

import edu.yu.einstein.wasp.plugin.bioanalyzer.plugin.BioanalyzerPlugin;
...
@ContextConfiguration(locations={"/bioanalyzer-test-launch-context.xml","/flows/bioanalyzer.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the BioanalyzerPlugin.FLOW_NAME batch job flow (defined in /flows/bioanalyzer.mainFlow.v1.xml)
 * @author 
 * 
 */
public class BioanalyzerJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(BioanalyzerJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the BioanalyzerPlugin.FLOW_NAME job flow. 
	...
	public void testSuccessfulBioanalyzerJobLaunch() throws Exception{
		try{
			...
			// The bioanalyzer flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(BioanalyzerPlugin.FLOW_NAME, jobParameters) );
			...
			// The BioanalyzerPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the BioanalyzerPlugin.FLOW_NAME job flow.
			...
			ExitStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(BioanalyzerPlugin.FLOW_NAME)).getExitStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleBioanalyzerStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleBioanalyzerStatusMessageTemplate.
		if (SimpleBioanalyzerStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Bioanalyzer messages only
	}
