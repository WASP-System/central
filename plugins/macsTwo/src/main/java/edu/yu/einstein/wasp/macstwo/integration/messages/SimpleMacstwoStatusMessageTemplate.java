 ...
 * 
 */
public class SimpleMacstwoStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleMacstwoStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, MacstwoMessageType.Macstwo);
	}
	
	public SimpleMacstwoStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(MacstwoMessageType.Macstwo))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct MacstwoMessageType
	 * @param message
	 * @return
	 */
	public static boolean isMessageOfCorrectType(Message<?> message) {
		return message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) &&  
				message.getHeaders().get(WaspMessageType.HEADER_KEY).equals(MacstwoMessageType.Macstwo);
	}
