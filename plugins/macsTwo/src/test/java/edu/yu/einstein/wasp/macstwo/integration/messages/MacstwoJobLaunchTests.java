...
import edu.yu.einstein.wasp.macstwo.plugin.MacstwoPlugin;

// The test context is created using the configuration files provided in the @ContextConfiguration locations list
@ContextConfiguration(locations={"/macstwo-test-launch-context.xml","/flows/macstwo.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the MacstwoPlugin.FLOW_NAME batch job flow (defined in /flows/macstwo.mainFlow.v1.xml)
 * @author 
 * 
 */
public class MacstwoJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(MacstwoJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the MacstwoPlugin.FLOW_NAME job flow. 
	...
	public void testSuccessfulMacstwoJobLaunch() throws Exception{
		try{
			...
			// The macstwo flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(MacstwoPlugin.FLOW_NAME, jobParameters) );
			...
			// The MacstwoPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the MacstwoPlugin.FLOW_NAME job flow.
			...
			BatchStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(MacstwoPlugin.FLOW_NAME)).getStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleMacstwoStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleMacstwoStatusMessageTemplate.
		if (SimpleMacstwoStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Macstwo messages only
	}
