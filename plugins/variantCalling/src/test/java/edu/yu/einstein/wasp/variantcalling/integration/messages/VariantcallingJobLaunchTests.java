 ...
 * 
 */
public class VariantcallingJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(VariantcallingJobLaunchTests.class);
	...
	public void testSuccessfulVariantcallingJobLaunch() throws Exception{
		...
		try{
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(VariantcallingPlugin.FLOW_NAME, jobParameters) );
			...
			ExitStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(VariantcallingPlugin.FLOW_NAME)).getExitStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleVariantcallingStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleVariantcallingStatusMessageTemplate.
		if (SimpleVariantcallingStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Variantcalling messages only
	}
