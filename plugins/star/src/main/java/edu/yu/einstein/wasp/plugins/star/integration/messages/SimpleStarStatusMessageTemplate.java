 ...
 * 
 */
public class SimpleStarStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleStarStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, StarMessageType.STAR);
	}
	
	public SimpleStarStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(StarMessageType.STAR))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct StarMessageType
	 * @param message
	 * @return
	 */
	public static boolean isMessageOfCorrectType(Message<?> message) {
		return message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) &&  
				message.getHeaders().get(WaspMessageType.HEADER_KEY).equals(StarMessageType.STAR);
	}
	
	@Override
	public SimpleStarStatusMessageTemplate getNewInstance(WaspStatusMessageTemplate messageTemplate){
		SimpleStarStatusMessageTemplate newTemplate = new SimpleStarStatusMessageTemplate();
