 ...
 * @author 
 */
package edu.yu.einstein.wasp.plugins.star.integration.messages;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

import edu.yu.einstein.wasp.plugins.star.integration.messages.SimpleStarStatusMessageTemplate;
import edu.yu.einstein.wasp.plugins.star.plugin.StarPlugin;
...
@ContextConfiguration(locations={"/star-test-launch-context.xml","/flows/star.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the StarPlugin.FLOW_NAME batch job flow (defined in /flows/star.mainFlow.v1.xml)
 * @author 
 * 
 */
public class StarJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(StarJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the StarPlugin.FLOW_NAME job flow. 
	...
	public void testSuccessfulStarJobLaunch() throws Exception{
		try{
			...
			// The star flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(StarPlugin.FLOW_NAME, jobParameters) );
			...
			// The StarPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the StarPlugin.FLOW_NAME job flow.
			...
			ExitStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(StarPlugin.FLOW_NAME)).getExitStatus();
