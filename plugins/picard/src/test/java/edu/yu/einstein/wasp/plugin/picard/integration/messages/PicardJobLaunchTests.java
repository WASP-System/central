...
import edu.yu.einstein.wasp.plugin.picard.plugin.PicardPlugin;
...
@ContextConfiguration(locations={"/picard-test-launch-context.xml","/flows/picard.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the PicardPlugin.FLOW_NAME batch job flow (defined in /flows/picard.mainFlow.v1.xml)
 * @author 
 * 
 */
public class PicardJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(PicardJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the PicardPlugin.FLOW_NAME job flow. 
	 ...
	 */
	@Test (groups = "picard-tests")
	public void testSuccessfulPicardJobLaunch() throws Exception{
		try{
			Map<String, String> jobParameters = new HashMap<String, String>();
			// The picard flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(PicardPlugin.FLOW_NAME, jobParameters) );
			...
			// The PicardPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the PicardPlugin.FLOW_NAME job flow.
			...
			BatchStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(PicardPlugin.FLOW_NAME)).getStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimplePicardStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimplePicardStatusMessageTemplate.
		if (SimplePicardStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Picard messages only
	}
