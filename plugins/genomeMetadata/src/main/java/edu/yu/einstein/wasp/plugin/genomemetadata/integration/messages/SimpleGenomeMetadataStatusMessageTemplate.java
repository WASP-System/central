 ...
 * 
 */
public class SimpleGenomeMetadataStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleGenomeMetadataStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, GenomeMetadataMessageType.GENOMEMETADATA);
	}
	
	public SimpleGenomeMetadataStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(GenomeMetadataMessageType.GENOMEMETADATA))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct GenomeMetadataMessageType
	 * @param message
	 * @return
	 */
	public static boolean isMessageOfCorrectType(Message<?> message) {
		return message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) &&  
				message.getHeaders().get(WaspMessageType.HEADER_KEY).equals(GenomeMetadataMessageType.GENOMEMETADATA);
	}
	
	@Override
	public SimpleGenomeMetadataStatusMessageTemplate getNewInstance(WaspStatusMessageTemplate messageTemplate){
		SimpleGenomeMetadataStatusMessageTemplate newTemplate = new SimpleGenomeMetadataStatusMessageTemplate();
