 ...
 * @author 
 */
package edu.yu.einstein.wasp.helptag.integration.messages;
 ...
 * 
 */
public class HelptagJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(HelptagJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the HelptagPlugin.FLOW_NAME job flow. 
	...
	public void testSuccessfulHelptagJobLaunch() throws Exception{
		try{
			...
			// The HelptagPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the HelptagPlugin.FLOW_NAME job flow.
			...
			WaspBatchExitStatus jobExecutionStatus = new WaspBatchExitStatus(jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(HelptagPlugin.PREP_FLOW_NAME)).getExitStatus());
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleHelptagStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleHelptagStatusMessageTemplate.
		if (SimpleHelptagStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Helptag messages only
	}
