 ...
 * 
 */
public class SimpleHelptagStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleHelptagStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, HelptagMessageType.HELPTAG);
	}
	
	public SimpleHelptagStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(HelptagMessageType.HELPTAG))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct HelptagMessageType
	 * @param message
	 * @return
	 */
