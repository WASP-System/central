 ...
 * @author 
 */
package edu.yu.einstein.wasp.plugin.bwameth.integration.messages;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

import edu.yu.einstein.wasp.plugin.bwameth.plugin.BwamethPlugin;
...
@ContextConfiguration(locations={"/bwameth-test-launch-context.xml","/flows/bwameth.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the BwamethPlugin.FLOW_NAME batch job flow (defined in /flows/bwameth.mainFlow.v1.xml)
 * @author 
 * 
 */
public class BwamethJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(BwamethJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the BwamethPlugin.FLOW_NAME job flow. 
	...
	public void testSuccessfulBwamethJobLaunch() throws Exception{
		try{
			Map<String, String> jobParameters = new HashMap<String, String>();
			// The bwameth flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(BwamethPlugin.FLOW_NAME, jobParameters) );
			...
			// The BwamethPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the BwamethPlugin.FLOW_NAME job flow.
			...
			ExitStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(BwamethPlugin.FLOW_NAME)).getExitStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleBwamethStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleBwamethStatusMessageTemplate.
		if (SimpleBwamethStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Bwameth messages only
	}
