 ...
 * 
 */
public class SimpleBwamethStatusMessageTemplate extends WaspStatusMessageTemplate {


	public SimpleBwamethStatusMessageTemplate() {
		super();
		addHeader(WaspMessageType.HEADER_KEY, BwamethMessageType.BWAMETH);
	}
	
	public SimpleBwamethStatusMessageTemplate(Message<WaspStatus> message){
		super(message);
	...
	public static boolean actUponMessage(Message<?> message, String task ){
		if (!message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) ||
				!((String) message.getHeaders().get(WaspMessageType.HEADER_KEY)).equals(BwamethMessageType.BWAMETH))
				return false;
		if (task == null)
			return true;
	...
	/**
	 * Returns true is the message is of the correct BwamethMessageType
	 * @param message
	 * @return
	 */
	public static boolean isMessageOfCorrectType(Message<?> message) {
		return message.getHeaders().containsKey(WaspMessageType.HEADER_KEY) &&  
				message.getHeaders().get(WaspMessageType.HEADER_KEY).equals(BwamethMessageType.BWAMETH);
	}
	
	@Override
	public SimpleBwamethStatusMessageTemplate getNewInstance(WaspStatusMessageTemplate messageTemplate){
		SimpleBwamethStatusMessageTemplate newTemplate = new SimpleBwamethStatusMessageTemplate();
