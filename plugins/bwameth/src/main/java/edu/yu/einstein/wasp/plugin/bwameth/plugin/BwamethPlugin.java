 ...
 * @author 
 */
package edu.yu.einstein.wasp.plugin.bwameth.plugin;
 ...
 * @author 
 */
public class BwamethPlugin extends WaspPlugin 
		implements 
			BatchJobProviding,
			ClientMessageI {
	...
	@Autowired
	@Qualifier("bwameth")
	private Software bwameth;
	
	public static final String FLOW_NAME = "edu.yu.einstein.wasp.plugin.bwameth.mainFlow";

	public BwamethPlugin(String iName, Properties waspSiteProperties, MessageChannel channel) {
	...
	private Message<String> helloWorldHelp() {
		String mstr = "\nBwameth plugin: hello world!\n" +
				"wasp -T bwameth -t helloWorld\n";
	...
	private Message<String> launchTestFlowHelp() {
		String mstr = "\nBwameth plugin: launch the test flow.\n" +
				"wasp -T bwameth -t launchTestFlow -m \'{id:\"1\"}\'\n";
		return MessageBuilder.withPayload(mstr).build();
	}
