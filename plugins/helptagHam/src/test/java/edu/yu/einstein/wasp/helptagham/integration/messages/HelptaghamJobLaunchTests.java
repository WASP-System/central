...
import edu.yu.einstein.wasp.helptagham.plugin.HelptaghamPlugin;
...
@ContextConfiguration(locations={"/helptagham-test-launch-context.xml","/flows/helptagham.mainFlow.v1.xml"})

/**
 * TestNG Test of Launching and successful completion of the HelptaghamPlugin.FLOW_NAME batch job flow (defined in /flows/helptagham.mainFlow.v1.xml)
 * @author 
 * 
 */
public class HelptaghamJobLaunchTests extends AbstractTestNGSpringContextTests implements MessageHandler {
	
	private final Logger logger = LoggerFactory.getLogger(HelptaghamJobLaunchTests.class);
	...
	/**
	 * This test involves sending a message to the remote wasp-daemon to initiate the HelptaghamPlugin.FLOW_NAME job flow. 
	...
	public void testSuccessfulHelptaghamJobLaunch() throws Exception{
		try{
			...
			// The helptagham flow doesn't actually require any job parameters but we'll add one for demonstration purposes. 
			...
			BatchJobLaunchMessageTemplate batchJobLaunchMessageTemplate = new BatchJobLaunchMessageTemplate( new BatchJobLaunchContext(HelptaghamPlugin.FLOW_NAME, jobParameters) );
			...
			// The HelptaghamPlugin.FLOW_NAME job flow sends two messages which we will catch and verify
			int repeats = 0;
			...
			// Check the receivedMessages list for receiving of the two messages sent by the HelptaghamPlugin.FLOW_NAME job flow.
			...
			ExitStatus jobExecutionStatus = jobExplorer.getMostRecentlyStartedJobExecutionInList(jobExplorer.getJobExecutions(HelptaghamPlugin.FLOW_NAME)).getExitStatus();
	...
	public void handleMessage(Message<?> messageIn) throws MessagingException {
		...
		// We can use static methods of the SimpleHelptaghamStatusMessageTemplate to check if the message is one we might be interested in.
		// In this case we just test whether the message was generated by the SimpleHelptaghamStatusMessageTemplate.
		if (SimpleHelptaghamStatusMessageTemplate.isMessageOfCorrectType(messageIn))
			receivedMessages.add(messageIn); // keep Helptagham messages only
	}
