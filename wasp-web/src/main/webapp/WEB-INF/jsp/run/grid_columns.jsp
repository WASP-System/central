...
<wasp:field name="statusForRun"  type="text" sortable="true" searchable="false" editable="false" columnWidth="90"/>
 ...
 _url='<c:url value="run/listJSON.do?selId=${param.selId}" />';
...
<wasp:field name="resourceName" type="text" sortable="true" showLink="true" baseLinkURL="<c:url value='resource/list.do' />" idCol="2"/>
<wasp:field name="machineId" type="hidden" />

<wasp:field name="flow_cell_name" type="text" showLink="true" baseLinkURL="<c:url value='facility/platformunit/selid/list.do' />" idCol="4" />
...
_addAttr['beforeShowForm'] = function(formId) {
	$('#resourceId').change(function () {      
		...
		if (!_val || !_val2) {
			...
			$.getJSON("<c:url value='run/samplesByResourceId.do' />",{resourceId: _val, ajax: 'true'}, function(data, textStatus, jqXHR){
	...
	$('#sampleId').change(function () {      
		...
		if (!_val || !_val2) {
			...
			var options = '<option value="-1"><fmt:message key="wasp.default_select.label" /></option>';
			$.getJSON("<c:url value='run/resourcesBySampleId.do' />",{sampleId: _val, ajax: 'true'}, function(data, textStatus, jqXHR){
				$.each(data, function (index, name) {                				    
					options += '<option value="' + index + '">' + name + '</option>';
				});
			})
