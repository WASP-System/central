	     ...
	     <sec:authorize access="hasRole('su') or hasRole('ft')">	     
		    <tr class="FormData">
		        <td colspan="3" align="left" class="submitBottom">	
		        	<a class="button" href="javascript:void(0);"  onclick='loadNewPageWithAjax("<c:url value="/job/${job.getId()}/sample/${sample.getId()}/sampledetail_ro.do" />");' ><fmt:message key="sampledetail_rw.cancel.label" /></a>
		        	<a class="button" href="javascript:void(0);"  onclick='loadNewPageWithAjax("<c:url value="/job/${job.getId()}/sample/${sample.getId()}/sampledetail_rw.do" />");' >Reset</a>
					<a class="button" href="javascript:void(0);"  onclick='postFormWithAjaxJson("editMacromoleculeFormId","<c:url value="/job/${job.getId()}/sample/${sample.getId()}/sampledetail_rw.do" />"); return false;' ><fmt:message key="sampledetail_rw.save.label" /></a>
		        </td>
		    </tr>
	    </sec:authorize>
	</table>
