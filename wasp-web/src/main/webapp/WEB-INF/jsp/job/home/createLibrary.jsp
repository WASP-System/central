	     ...
	     <sec:authorize access="hasRole('su') or hasRole('ft')">
		    <tr class="FormData">
			    <td colspan="3" align="left" class="submitBottom">
		        	<a class="button" href="javascript:void(0);"  onclick='loadNewPageWithAjax("<c:url value="/job/${job.getId()}/samples.do" />");' ><fmt:message key="sampledetail_rw.cancel.label" /></a>
		        	<a class="button" href="javascript:void(0);"  onclick='loadNewPageWithAjax("<c:url value="/job/${job.getId()}/macromolecule/${macromoleculeSample.getId()}/createLibrary.do" />");' >Reset</a>
		        	<a class="button" href="javascript:void(0);"  onclick='postFormWithAjaxJson("createLibraryFormId","<c:url value="/job/${job.getId()}/macromolecule/${macromoleculeSample.getId()}/createLibrary.do" />"); return false;' ><fmt:message key="sampledetail_rw.save.label" /></a>	
			    </td>
			</tr>
	     </sec:authorize>
	</table>
