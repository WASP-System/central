...
<table class="data" style="margin: 0px 0">
	...
	<c:if test='${fn:length(userSubmittedJobCommentsList) > 0}'>
		...
		<tr>
			<td style="padding-top:20px;padding-bottom:20px;padding-left:10px;padding-right:20px; " width="20%"><fmt:formatDate value="${userSubmittedJobCommentsList[0].getDate()}" pattern="MM-dd-yyyy" /></td>
			...
			<td style="padding-top:20px;padding-bottom:20px;padding-left:20px;padding-right:10px; " width="80%"><c:out value="${userSubmittedJobCommentsList[0].getValue()}" escapeXml="false"  /></td>
		</tr>
	</c:if>
	...
	<c:if test='${fn:length(facilityJobCommentsList) > 0}'>
		...
		<tr>
			...
			<%-- for potential problems with escapeXML in next c:out see: http://www.coderanch.com/t/535302/JSP/java/Keeping-line-breaks-String --%>
			<td style="padding-top:20px;padding-bottom:20px;padding-left:20px;padding-right:10px; " width="80%"><c:out value="${facilityJobComment.getValue()}" escapeXml="false"  /></td>
		</tr>
		</c:forEach>
	</c:if>
