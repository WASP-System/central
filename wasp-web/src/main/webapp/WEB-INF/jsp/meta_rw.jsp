...
<c:forEach items="${_metaList}" var="_meta" varStatus="status">
	...
	<c:if test="${_meta.property.formVisibility != 'ignore'}">
		...
		<c:choose>
			<c:when test="${_meta.property.formVisibility == 'hidden'}">
				<tr colspan="3" id="row_${id}" style="display:none"><td>
					<input type="hidden" name="${_area}Meta_${_meta.k}" id="${id}"  value="${_meta.v}" />
				</td></tr>
			</c:when>
		<c:otherwise>
