...
<input class="ui-widget ui-widget-content fm-button" type="button" value="<fmt:message key="sample.controlLib_createButton.label" />" onclick='location.href="<c:url value="/sample/createUpdateLibraryControl/0.do" />"' />
</c:when>
<c:otherwise>
<input class="ui-widget ui-widget-content fm-button" type="button" value="<fmt:message key="sample.controlLib_createButton.label" />" onclick='location.href="<c:url value="/sample/createUpdateLibraryControl/0.do" />"' />
...
<c:forEach items="${controlLibraryList}" var="controlLibrary">
	...
	<tr>
		<td><c:out value="${controlLibrary.getName()}"/> <a href="<c:url value="/sample/createUpdateLibraryControl/${controlLibrary.getSampleId()}.do" />">[<fmt:message key="sample.controlLib_edit.label" />]</a></td>
