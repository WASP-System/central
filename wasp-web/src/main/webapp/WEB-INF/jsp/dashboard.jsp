...
<wasp:message />
<div>
	<p>Welcome back ${me.firstName} ${me.lastName}</p>
</div>
<div>
	...
	<div>
		...
		<p>
			...
			<c:choose>
				...
				<c:otherwise>
					<span style="color: red"> (<c:out value="${departmentAdminPendingTasks}" /> Pending Department Administrator Task<c:if test='${departmentAdminPendingTasks != 1}'>s</c:if>)
					</span>
				</c:otherwise>
			</c:choose>
		</p>
		...
		<c:forEach items="${departments}" var="d">
			<div>
				...
				<div>
					<a href="<c:url value="/department/pendinglab/list/${departmentId}.do"/>">Pending Lab Approval</a>
				</div>
				<div>
		...
		<sec:authorize access="hasRole('su') or hasRole('ga')">
			<div>
				...
				<c:choose>
					...
					<c:otherwise>
						<span style="color: red"> (<c:out value="${allLabManagerPendingTasks}" /> Pending PI/Lab Manager Task<c:if test='${allLabManagerPendingTasks != 1}'>s</c:if>) </span>
					</c:otherwise>
				</c:choose>
			</div>
			<br />
				...
				<sec:authorize access="hasRole('lm-${l.labId}' )">
					...
					<div>
						...
						<c:choose>
							...
							<c:otherwise>
								<span style="color: red"> (<c:out value="${labmap.get(l.labId)}" /> Pending PI/Lab Manager Task<c:if test='${labmap.get(l.labId) != 1}'>s</c:if>) </span>
							</c:otherwise>
						</c:choose>
					</div>
				</sec:authorize>
