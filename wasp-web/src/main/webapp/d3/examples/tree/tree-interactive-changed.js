...
d3.json("http://localhost:8080/wasp/autocomplete/getJobSampleTreeJson.do?jobId=1", function(json) {
//d3.json("http://localhost:8080/wasp/autocomplete/getInstitutesForDisplay.do", function(json) {
...
function clickLabel(d) {
  labelClick = true;
  if (d.depth==1) {
	  d3.select('#Job-div').style("display", "none");
  d3.select('#Sample-div').style("display", "block") ;
  } else {
	  d3.select('#Job-div').style("display", "block") ;
  d3.select('#Sample-div').style("display", "none") ;
  }
}
