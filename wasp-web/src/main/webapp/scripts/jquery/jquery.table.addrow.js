 ...
 *
 * note by dubin
 * for use with new wasp (jquery newer versions: modified the three .live calls to .on() according to http://www.west-wind.com/weblog/posts/2013/Jun/12/Replacing-jQuerylive-with-jQueryon
 * this was needed as new jquery no longer supports live().
 * Also needed to reference container_div_for_adding_rows, which is a div surrounding the table to which rows are added
 */
(function($){if(typeof $.fn.btnAddRow== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			var t=this;
			this.update();
			$("#container_div_for_adding_rows")
			.on("click", ".addRow"+this.seed, function(){
				var newRow=t.addRow();
			});
			$("#container_div_for_adding_rows")
			.on("click", ".delRow"+this.seed, function(){
			...
			$("#container_div_for_adding_rows")
			.on("keyup", ".autoAdd"+this.seed, function(){
...
//below is the original file, which no longer works with newest version of jquery
//so no longer used. just here for reference purposes.
