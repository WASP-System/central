...
public class JobController extends WaspController {
  ...
  @RequestMapping(value = "/pendingJobApprovalByPIorLM/{labId}.do", method = RequestMethod.POST)
  @PreAuthorize("hasRole('su') or hasRole('sa') or hasRole('ga') or hasRole('fm') or hasRole('ft') or hasRole('lm-' + #labId) or hasRole('pi-' + #labId)")
	public String pendingJobApprovalByPIorLM(
			@PathVariable("labId") Integer labId, 
			@RequestParam("jobId") Integer jobId, 
	  ...
	  try{
		  if(!comment.trim().isEmpty()){
			  jobService.setJobApprovalComment("piApprove",job.getJobId(), comment.trim());
		  }
	  }
	  catch(Exception e){
		  logger.warn(e.getMessage());
...
/*  replaced by /pendingJobApprovalByPIorLM/{labId}.do
